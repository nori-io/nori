...
func (g *directedGraph) AddNode(id meta.ID) error {
...
func (g *directedGraph) RemoveNode(id meta.ID) {
...
func (g *directedGraph) Has(id meta.ID) bool {
	_, ok := g.nodes[id]

	return ok
}

func (g *directedGraph) Node(id meta.ID) meta.ID {
	return g.nodes[id]
}

func (g *directedGraph) Nodes() []meta.ID {
...
func (g *directedGraph) From(id meta.ID) []meta.ID {
...
func (g *directedGraph) Edge(from, to meta.ID) Edge {
...
func (g *directedGraph) HasEdgeFromTo(from, to meta.ID) bool {
	if _, ok := g.nodes[from]; !ok {
		return false
	}
...
func (g *directedGraph) To(id meta.ID) []meta.ID {
	if _, ok := g.from[id]; !ok {
		return nil
	}
...
func (g *directedGraph) NewEdge(from, to meta.ID) Edge {
...
func (g *directedGraph) Sort() ([]meta.ID, error) {
	var sorted []meta.ID
